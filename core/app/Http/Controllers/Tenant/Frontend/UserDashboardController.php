...
class UserDashboardController extends Controller
{
    ...
    public function user_index(){
        ...
        {
            $restaurant_orders = [
                "total_orders" => MenuOrder::where('user_id',$this->logged_user_details()->id)->where('status', '!=', 3)->paginate(10),
                "accepted_orders" => MenuOrder::where('user_id',$this->logged_user_details()->id)->where('status',1)->count(),
                "canceled_orders" => MenuOrder::where('user_id',$this->logged_user_details()->id)->where('status',3)->count(),
                "pending_orders" => MenuOrder::where('user_id',$this->logged_user_details()->id)->where('status',0)->count(),
                "inprogress_orders" => MenuOrder::where('user_id',$this->logged_user_details()->id)->where('status',2)->count(),
            ];
        }
